...
Singleton {
    ...
    property string currentConditionDescription: ""
    ...
    readonly property string weatherCacheDirectory: Quickshell.cachePath("weather_images")
    readonly property string locationConfigFilePath: Quickshell.cachePath("location")
    ...
    function updateLocalBackground() {
        ...
        backgroundCheckProcess.command = ["test", "-f", localPath];
    ...
    Process {
        ...
        onExited: (code) => {
            ...
            if (code === 0) {
                ...
                backgroundDownloadProcess.command = ["curl", "-L", "--create-dirs", root.weatherBackgroundMap[condition], "-o", localPath];
    ...
    Process { 
        ...
        onExited: (code) => {
            if (code === 0) root.updateLocalBackground();
        }
    }

    onCurrentConditionChanged: updateLocalBackground()
            ...
            if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
                try {
                    ...
                    if (data && data.address) {
                        ...
                        root.currentCityName = address.city || address.town || address.village || address.municipality || address.county || address.state || address.country || "Unknown City";
                    }
                } catch (error) {
                    root.currentCityName = "Unknown City";
                }
            }
        }
        xhr.open("GET", url);
    ...
    function updateLocation(newLocation) {
        if (!newLocation || !newLocation.includes(",")) return;
        let sanitizedLocation = newLocation.replace(/[^0-9.,-]/g, "");
        if (!sanitizedLocation.includes(",")) return;
                ...
                if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
                    try {
                        ...
                        if (data && data.current) {
                            ...
                            root.currentCondition = mapWeatherCodeToCondition(weatherCode);
                            root.currentConditionDescription = mapWeatherCodeToDescription(weatherCode);
                            root.currentTemperature = data.current.temperature_2m;
                            
                            let iconCodeMap = { "Clear": "01d", "Clouds": "03d", "Rain": "10d", "Snow": "13d", "Thunderstorm": "11d" };
                            let iconCode = iconCodeMap[root.currentCondition] || "01d";
                            root.currentWeatherIconUrl = `https://openweathermap.org/img/wn/${iconCode}@2x.png`;
                        }
                    } catch (error) {}
                }
            }
    ...
    Timer { 
        interval: ThemeManager.weatherUpdateInterval 
        running: true 
        repeat: true 
        triggeredOnStart: false
        onTriggered: root.fetchLatestWeather() 
    }
}
